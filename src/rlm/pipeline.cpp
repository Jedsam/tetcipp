...
void Pipeline::createGraphicsPipeline(
  ...
  // It is also possible to use other render passes with this pipeline instead
  // of this specific instance, but they have to be compatible with renderPass.
  // The requirements for compatibility are described here :
  ...
  // that is about to be created by index with basePipelineIndex. These values
  // are only used if the VK_PIPELINE_CREATE_DERIVATIVE_BIT flag is also
  ...
  // to store and reuse data relevant to pipeline creation across multiple calls
  // to vkCreateGraphicsPipelines and even across program executions if the
  // cache is stored to a file. This makes it possible to significantly speed up
  // pipeline creation at a later time.
...
void Pipeline::createShaderModule(
  ...
  auto result = vkCreateShaderModule(
      rlmDevice.getDevice(), &createInfo, nullptr, shaderModule);
  if (result != VK_SUCCESS) {
...
void Pipeline::defaultPipelineConfigInfo(PipelineConfigInfo &configInfo) {
  ...
  // Specifieswhat kind of geometry will be drawn from the vertices and if
  // primitive restart should be enabled. The topology member can have values
  // like:
  //
  ...
  // is useful in some special cases like shadow maps. Using this requires
  ...
  // -VK_POLYGON_MODE_POINT: polygon vertices are drawn as points Using any mode
  // other than fill requires enabling  a GPU feature.
  ...
  // you to enable the wideLines GPU feature. to be setup later
  ...
  // biasing them based on a fragment's slope. This is sometimes used for shadow
  // mapping.
  ...
  // Configure a setting which is about multiple sampling, one of the ways to
  // perform anti-aliasing. It works by combining the fragment shader results of
  // multiple polygons that rasterize to the same pixel. This mainly occurs
  // along edges, which is also where the most noticeable aliasing artifacts
  // occur. Because it doesn't need to run the fragment shader multiple times if
  // only one polygon maps to a pixel, it is significantly less expensive than
  ...
  // blending. The operations that will be performed are best demonstrated using
  ...
  configInfo.colorBlendAttachment.blendEnable =
      VK_TRUE;  // Whether to enable the operation
