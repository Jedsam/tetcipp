...
SwapChain::SwapChain(Device &rlmDevice, VkExtent2D windowExtent)
    : rlmDevice{rlmDevice}, swapChainExtent{windowExtent} {
  init();
}
...
void SwapChain::init() {
  createSwapChain();
  spdlog::debug("SwapChain::init: Created swap chain");
  createImageViews();
  spdlog::debug("SwapChain::init: Created image views");
  createRenderPass();
  spdlog::debug("SwapChain::init: Created render pass");
  createFramebuffers();
  spdlog::debug("SwapChain::init: Created frame buffers");
}
...
void SwapChain::createFramebuffers() {
  ...
  spdlog::debug(
      "SwapChain::createFramebuffers: Number of image views: {}",
...
void SwapChain::createRenderPass() {
  ...
  // does not occur at the right time. It assumes that the transition occurs at
  // the start of the pipeline, but we haven't acquired the image yet at that
  // point! So we will add a dependency to make he render pass wait for the
  ...
  // implicit subpass before or after the render pass depending on whether it is
  // specified in srcSubpass or dstSubpass. The index 0 refers to our subpass,
  // which is the first and only one. The dstSubpass must always be higher than
  // srcSubpass to prevent cycles in the dependency graph (unless one of the
  ...
  // which these operations occur. We need to wait for the swap chain to finish
  // reading from the image before we can access it. This can be accomplished by
  ...
  // attachment -VK_ATTACHMENT_LOAD_OP_CLEAR: Clear the values to a constant at
  // the start -VK_ATTACHMENT_LOAD_OP_DONT_CARE: Existing contents are
  // undefined; we don't care about them
  ...
  // and can be read later -VK_ATTACHMENT_STORE_OP_DONT_CARE: Contents of the
  ...
  // a certain pixel format, however the layout of the pixels in memory can
  // change based on what you're trying to do with an image. Some of the most
  // common layouts are:
  ...
  // index in the attachment descriptions array The layout specifies which
  // layout we would like the attachment to have during a subpass that uses this
  // reference. Vulkan will automatically transition the attachment to this
  // layout when the subpass is started. We intend to use the attachment to
  // function as a color buffer and the VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
  ...
  auto result = vkCreateRenderPass(
      rlmDevice.getDevice(), &renderPassInfo, nullptr, &renderPass);
  if (result != VK_SUCCESS) {
...
void SwapChain::createImageViews() {
  ...
  for (size_t i = 0; i < swapChainImages.size(); i++) {
    ...
    auto result = vkCreateImageView(
        rlmDevice.getDevice(), &createInfo, nullptr, &swapChainImageViews[i]);
    if (result != VK_SUCCESS) {
      throw std::runtime_error("failed to create image views!");
    }
  }
  spdlog::debug(
      "SwapChain::createImageViews: Created {} image views",
      swapChainImageViews.size());
}
...
void SwapChain::createSwapChain() {
  ...
  spdlog::debug("SwapChain::createSwapChain: Chosen swap surface format");
  VkPresentModeKHR presentMode =
      chooseSwapPresentMode(swapChainSupport.presentModes);
  spdlog::debug("SwapChain::createSwapChain: Chosen swap present mode");
  VkExtent2D extent = chooseSwapExtent(swapChainSupport.capabilities);
  spdlog::debug("SwapChain::createSwapChain: Chosen swap extent");
  ...
  createInfo.clipped =
      VK_TRUE;  // False if you want to get access to the pixels behind a window

  auto result = vkCreateSwapchainKHR(
      rlmDevice.getDevice(), &createInfo, nullptr, &swapChain);
  if (result != VK_SUCCESS) {
    throw std::runtime_error("failed to create swap chain!");
  }

  vkGetSwapchainImagesKHR(
      rlmDevice.getDevice(), swapChain, &imageCount, nullptr);
  swapChainImages.resize(imageCount);
  vkGetSwapchainImagesKHR(
      rlmDevice.getDevice(), swapChain, &imageCount, swapChainImages.data());
}

VkSurfaceFormatKHR SwapChain::chooseSwapSurfaceFormat(
    const std::vector<VkSurfaceFormatKHR> &availableFormats) {
  auto it =
      std::ranges::find_if(availableFormats, [](const auto &availableFormat) {
        return (
